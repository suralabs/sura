    ...
    <div class="card-body">
        <div class="media">
            <div class="avatar {{ $row['ava_online'] }} mr-5">
                <img src="{{ $row['ava'] }}" alt="{{ $row['name'] }}" class="avatar-img rounded-circle" style="width: 60px;height: 60px;"
            ...
            <div class="media-body align-self-center">
                <h4 class="mb-1">
                    <a href="/u{{ $row['user_id'] }}" onClick="Page.Go(this.href); return false"
                       onmouseover="wall.showTag({{ $row['user_id'] }}, {{ $row['user_id'] }}, 1)" onmouseout="wall.hideTag({{ $row['user_id'] }}, {{ $row['user_id'] }}, 1)">{{ $row['name'] }}</a>
                </h4>
